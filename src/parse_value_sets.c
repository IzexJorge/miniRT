...
int	ft_parse_color(char *input, int line, t_color *color)
{
	...
	if (ft_str_arraylen(values) != 3)
		return (ft_err_free("Color needs 2 commas.", line, values), 2);
...
int	ft_parse_coord(char *input, int line, t_coord *coordinates)
{
	...
	if (values == NULL)
		return (perror("error"), 1);
	if (ft_str_arraylen(values) != 3)
		return (ft_err_free("Coordinates need 2 commas.", line, values), 2);
...
int	ft_parse_orient(char *input, int line, t_coord *orientation)
{
	...
	if (values == NULL)
		return (perror("error"), 1);
	if (ft_str_arraylen(values) != 3)
		return (ft_err_free("Orientation needs 2 commas.", line, values), 2);

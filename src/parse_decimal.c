...
/*   Updated: 2026/02/03 12:16:24 by jescuder         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_get_frac_part(char **input, double *var)
{
	...
	while (ft_isdigit(**input))
	{
		if (ft_is_greater(divisor, DBL_MAX / 10.0))
			return (1);
		divisor *= 10.0;
...
int	ft_parse_decimal(char *input, int line, char *field, double *var)
{
	...
	{
		if (err_code == 1)
			ft_err_range(line, field);
		else if (err_code == 2)
			ft_err_field(line, field,
				"Fractional part is smaller than the internal precision(1e-8)");
		else if (err_code == 3)
			ft_err_field(line, field, "Incorrect character in number");
		return (2);
	}
	if (!ft_strcmp(field, "Color"))
		*var = *var / 255.0;
	if (!ft_strcmp(field, "FOV"))
		*var = *var * M_PI / 180.0;
	return (0);
}
